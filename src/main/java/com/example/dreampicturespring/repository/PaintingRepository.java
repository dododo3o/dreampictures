...
public interface PaintingRepository extends JpaRepository<Paintingtbl, Integer> {
    @Query(value = "select paintingtbl.no_painting,membershiptbl.img,membershiptbl.nickname,paintingtbl.pname,paintingtbl.url from paintingtbl, membershiptbl where paintingtbl.no_membership = membershiptbl.no_membership and paintingtbl.status =  0", nativeQuery = true)
